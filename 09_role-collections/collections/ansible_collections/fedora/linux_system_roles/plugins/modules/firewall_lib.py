...
def _recursive_diff_to_added_removed(recursive_result):
    """Normalize recursive_show_diffs() output into added/removed dicts."""
    if not recursive_result:
        return {"added": {}, "removed": {}}
    removed = recursive_result.get("removed")
    added = recursive_result.get("added")
    if isinstance(removed, dict) and isinstance(added, dict):
        return {
            "added": copy.deepcopy(added),
            "removed": copy.deepcopy(removed),
        }
    return {
        "added": copy.deepcopy(added) if added is not None else {},
        "removed": copy.deepcopy(removed) if removed is not None else {},
    }
...
def get_diffs(
    ...
    if (
        ...
        diff["permanent"]["removed"]["allow_zone_drifting"] = (
            original_firewalld_conf.get("allow_zone_drifting")
        )
        diff["permanent"]["added"]["allow_zone_drifting"] = current_firewalld_conf.get(
            "allow_zone_drifting"
        )
    return diff
...
def check_for_diffs(
    ...
    changed = any(
        (
            diff["permanent"]["added"],
            diff["permanent"]["removed"],
            diff["runtime"]["added"],
            diff["runtime"]["removed"],
        )
    )
